...
public class TestInvokerServierController {
	...
	public String getMetadata(){
		List<ServiceInstance> instances = discoveryClient.getInstances("HeartBeatEurekaProvider");
		for(ServiceInstance sie :instances){
			String userName = sie.getMetadata().get("userName");
			String corpName = sie.getMetadata().get("corpName");
			System.out.println(sie.getPort()+"-----"+userName +"----"+corpName);
		}
		return "";
	}
}
